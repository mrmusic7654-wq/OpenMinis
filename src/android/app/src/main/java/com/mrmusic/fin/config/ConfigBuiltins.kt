package com.mrmusic.fin.config

import android.content.Context
import com.mrmusic.fin.config.collections.EnvVarsCollection
import com.mrmusic.fin.service.DynamicIslandSupport
import com.mrmusic.fin.config.collections.GroupsCollection
import com.mrmusic.fin.config.collections.ModelsCollection
import com.mrmusic.fin.config.collections.ProvidersCollection
import com.mrmusic.fin.config.fields.ClosureField
import com.mrmusic.fin.config.fields.PrefsBoolField
import com.mrmusic.fin.config.fields.PrefsDoubleField
import com.mrmusic.fin.config.fields.PrefsEnumField
import com.mrmusic.fin.config.fields.PrefsIntCodedEnumField
import com.mrmusic.fin.config.fields.PrefsIntField
import com.mrmusic.fin.config.fields.PrefsLongField
import com.mrmusic.fin.config.fields.PrefsStringField
import com.mrmusic.fin.config.fields.ReadOnlyField
...
import com.mrmusic.fin.ui.chat.ChatViewModelStore
...
internal object ConfigBuiltins {
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        // [T-app-themes] Visual theme — string id so the catalogue can grow
        // without renumbering. Mirrors iOS `appearance.app_theme`.
        r.register(
            PrefsEnumField(
                path = "appearance.app_theme",
                displayName = "Visual theme",
                description = "Colour theme applied on top of light/dark: " +
                    com.mrmusic.fin.ui.theme.AppTheme.ids.joinToString(" / ") + ".",
                prefs = prefs,
                key = com.mrmusic.fin.ui.theme.AppTheme.KEY_APP_THEME,
                cases = com.mrmusic.fin.ui.theme.AppTheme.ids,
                defaultValue = com.mrmusic.fin.ui.theme.AppTheme.DEFAULT.id,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = com.mrmusic.fin.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        val appearancePrefs = context.getSharedPreferences(
            com.mrmusic.fin.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsBoolField(
                ...
                key = com.mrmusic.fin.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.mrmusic.fin.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.mrmusic.fin.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.mrmusic.fin.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(com.mrmusic.fin.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.mrmusic.fin.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(com.mrmusic.fin.config.collections.ThinkingRulesCollection(providerRepo))
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, com.mrmusic.fin.data.model.ProviderInstance>(cfg.instances.size)
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(com.mrmusic.fin.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.mrmusic.fin.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, com.mrmusic.fin.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, com.mrmusic.fin.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [com.mrmusic.fin.agent.SoulStore.isOverLimit].
        ...
        fun loadCurrent(): com.mrmusic.fin.agent.SoulFile =
            com.mrmusic.fin.agent.SoulStore.load(context)
                ?: com.mrmusic.fin.agent.SoulMDParser.parse(
                    com.mrmusic.fin.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: com.mrmusic.fin.agent.SoulFile) {
            com.mrmusic.fin.agent.SoulStore.save(context, file)
        }
        ...
        r.register(
            ClosureField(
                ...
                description = "The agent's name as shown in chat bubble headers and as the system-prompt identity. Trimmed; empty falls back to \"Fin\".",
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        com.mrmusic.fin.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        com.mrmusic.fin.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${com.mrmusic.fin.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${com.mrmusic.fin.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (com.mrmusic.fin.agent.SystemPromptBuilder
                    ...
                    val check = com.mrmusic.fin.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is com.mrmusic.fin.agent.SoulBodyLimitCheck.Ok -> Unit
                        is com.mrmusic.fin.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${com.mrmusic.fin.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is com.mrmusic.fin.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${com.mrmusic.fin.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()
