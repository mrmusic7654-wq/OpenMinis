...
import com.mrmusic.fin.ui.components.DialogTextField
...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.mrmusic.fin.data.repository.EnvVarRepository? = null,
) {
    ...
    if (deleteId != null) {
        ...
        AlertDialog(
            ...
            dismissButton = {
                FinTextButton(onClick = { deleteId = null }) { Text(stringResource(R.string.cancel)) }
            },
        )
    }
}
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.mrmusic.fin.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.mrmusic.fin.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.mrmusic.fin.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.mrmusic.fin.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.mrmusic.fin.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.mrmusic.fin.mcp.oauth.MCPOAuthConfig(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (oauthAuthorized) {
                        FinTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.mrmusic.fin.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.mrmusic.fin.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.mrmusic.fin.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.mrmusic.fin.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.mrmusic.fin.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.mrmusic.fin.mcp.oauth.MCPOAuthController.Result.Failed -> {
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            FinTextButton(onClick = {
                val server = buildServerFromForm() ?: return@FinTextButton
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            if (isEdit) {
                FinTextButton(onClick = onRequestDelete) {
                    Text(stringResource(R.string.delete), color = MaterialTheme.colorScheme.error)
                }
            }
            Spacer(Modifier.weight(1f))
            FinTextButton(onClick = onDone) { Text(stringResource(R.string.cancel)) }
            FinTextButton(onClick = {
                val server = buildServerFromForm() ?: return@FinTextButton
    ...
    if (showClearEnvConfirm) {
        AlertDialog(
            ...
            dismissButton = {
                FinTextButton(onClick = { showClearEnvConfirm = false }) {
