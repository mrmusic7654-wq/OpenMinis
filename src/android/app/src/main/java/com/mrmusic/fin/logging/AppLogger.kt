package com.mrmusic.fin.logging
...
object AppLogger {
     ...
     *
     * [init] is called from FinApp.onCreate AFTER the safe-mode
    ...
    private fun startCapture() {
        ...
        // post-crash one.
        appContext?.let { com.mrmusic.fin.diagnostics.EnvironmentBanner.log(it) }
    }
     ...
     * itself produced (tag prefix `Fin.`) are skipped — [log] already wrote
    ...
    private fun writeLogcatLine(rawLine: String) {
        ...
        if (slashIdx >= 0 && parenIdx > slashIdx) {
            ...
            if (tag.startsWith("Fin.") || tag == "AppLogger") return
        }
        try {
            val now = Date()
