package com.mrmusic.fin.scheduled
...
import com.mrmusic.fin.debug.HeadlessChatRunner
import com.mrmusic.fin.logging.AppLogger
import com.mrmusic.fin.service.AgentForegroundService
...
object ScheduledAgentRunner {
     ...
     *   target chat gone, FinApp not initialized).
     */
    suspend fun run(
        context: Context,
        task: ScheduledTask,
        ...
        // [T-android-scheduled-lateinit-crash-156] `as? FinApp` only rules out
        ...
        val app = context.applicationContext as? FinApp ?: run {
            AppLogger.error(TAG, "Application is not FinApp — skipping task ${task.id}")
            return null
        }
        if (!app.subsystemsReady()) {
            AppLogger.error(
                TAG,
                "FinApp subsystems not initialized (safe-mode or failed init) — skipping task ${task.id}",
            )
            return null
        }
    ...
    private suspend fun resolveSessionId(app: FinApp, task: ScheduledTask): String? {
        return when (val mode = task.targetMode) {
            ...
            ScheduledTargetMode.NewSession -> {
                ...
                val memoryOn = com.mrmusic.fin.data.MemoryGlobalPrefs.isGlobalEnabled(app)
    ...
    private fun postCompletionNotification(
        ...
        val title = "Fin: ${task.label.ifBlank { "Scheduled task" }}"
