    ...
    private fun backgroundError(args: OffloadArgs) = NativeOffloadResult(
        ...
        OffloadOutput.formatBody(
            JSONObject().put("error", "clipboard_requires_foreground")
                .put("message", "Android 10+ blocks clipboard access when the app is not in the foreground. Ask the user to bring Fin to the foreground and retry.")
                .toString(),
            args,
        ) + "\n",
    )
...
OPTIONS:
  ...
  --label <name>       Optional ClipData label (default: Fin)
