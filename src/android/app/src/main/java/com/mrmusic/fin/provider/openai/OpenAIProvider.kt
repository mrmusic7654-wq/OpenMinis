package com.mrmusic.fin.provider.openai
...
import com.mrmusic.fin.data.model.LLMMediaAttachment
import com.mrmusic.fin.data.model.LLMMessage
import com.mrmusic.fin.data.model.LLMModel
import com.mrmusic.fin.data.model.LLMResponse
...
import com.mrmusic.fin.provider.thinking.ThinkingResolveContext
import com.mrmusic.fin.provider.thinking.ThinkingRuleResolver
import com.mrmusic.fin.provider.LLMProvider
import com.mrmusic.fin.provider.applyUserAgentOverride
...
import com.mrmusic.fin.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.mrmusic.fin.provider.thinking.ThinkingRuleResolver]'s
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.mrmusic.fin.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.mrmusic.fin.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.mrmusic.fin.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.mrmusic.fin.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.mrmusic.fin.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.mrmusic.fin.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.mrmusic.fin.BuildConfig.DEBUG) {
                com.mrmusic.fin.debug.LLMRequestLog.add(
                    com.mrmusic.fin.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.mrmusic.fin.logging.AppLogger.debug(
                                "OpenAIProvider",
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.mrmusic.fin.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private suspend fun buildRequest(bodyStr: String): Request {
        ...
        if (isOAuth && !forceChatCompletions) {
            ...
            // Fin UA here: the ChatGPT OAuth backend validates the
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.mrmusic.fin.provider.ImageBudget.compressUnderBudget(data)
    ...
    private fun buildResponsesAPIBody(
        ...
        if (com.mrmusic.fin.data.FastModePrefs.isEnabled() &&
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.mrmusic.fin.logging.AppLogger.error(
                                        "OpenAIProvider",
