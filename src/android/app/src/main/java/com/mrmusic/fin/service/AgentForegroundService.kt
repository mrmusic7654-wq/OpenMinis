package com.mrmusic.fin.service
...
class AgentForegroundService : Service() {
    ...
    companion object {
        ...
        private const val ACTION_STOP = "com.mrmusic.fin.STOP_AGENT_SERVICE"
    ...
    private fun buildNotification(sessionCount: Int, toolStatus: String): Notification {
        ...
        val mainIntent = Intent(this, Class.forName("com.mrmusic.fin.MainActivity")).apply {
        ...
        // Title:   "Fin is using <Tool>"  (or session-count summary when idle/between turns)
        ...
        // through to the generic "Fin is running" title while the icon fell
        ...
        // by the system with no Activity, so it can observe a FinApp whose
        ...
        val minisApp = (applicationContext as? FinApp)?.takeIf { it.subsystemsReady() }
    ...
    private fun toolDisplayLabel(toolName: String): String = when (toolName) {
        "shell_execute" -> "Fin is using Shell"
        "file_read" -> "Fin is reading File"
        "file_write" -> "Fin is using Editor"
        "file_edit" -> "Fin is editing File"
        "browser_use" -> "Fin is using Browser"
        "read_image" -> "Fin is reading Image"
        "memory_write", "memory_get" -> "Fin is using Memory"
        "web_search" -> "Fin is using Search"
        else -> "Fin is using $toolName"
    }
