package com.mrmusic.fin.provider.thinking
...
object ThinkingRuleResolver {
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = com.mrmusic.fin.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to com.mrmusic.fin.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = com.mrmusic.fin.provider.anthropic.AnthropicProvider
