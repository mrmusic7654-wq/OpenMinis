...
import com.mrmusic.fin.offload.FinNotificationListenerService
import com.mrmusic.fin.offload.OffloadPermissionManager
import com.mrmusic.fin.offload.ScheduledNotificationReceiver
import com.mrmusic.fin.offload.ScheduledNotificationStore
...
class NotificationOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun handleSend(args: OffloadArgs): NativeOffloadResult {
                ...
                if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                    ...
                    r = OffloadPermissionManager.requestSettingsGate(
                        OffloadPermissionManager.SettingsGateRequest(
                            ...
                            message = "Fin needs notification permission to send notifications. Open Settings to allow it.",
        ...
        if (FinNotificationListenerService.isEnabled(context) &&
            FinNotificationListenerService.isConnected()
        ) {
            val seen = FinNotificationListenerService.awaitPosted(context.packageName, notifId)
            if (!seen) {
        ...
        val oemHint = if (OsCompat.isHuawei || OsCompat.isXiaomi) {
            "On ${OsCompat.oemLabel()}, banner notifications may be disabled by default — the user can enable them in Settings → Notifications for Fin."
        } else null
        val data = JSONObject()
            .put("id", id)
            .put("title", title)
    ...
    private fun handleSettings(args: OffloadArgs): NativeOffloadResult {
        ...
        data.put("listener_access", FinNotificationListenerService.isEnabled(context))
        AppLogger.info(TAG, "settings: can_post=$canPost listener=${FinNotificationListenerService.isEnabled(context)}")
        return NativeOffloadResult(0, OffloadOutput.formatBody(data.toString(2), args) + "\n")
    }
    ...
    private fun handleList(args: OffloadArgs): NativeOffloadResult {
            ...
            val result = runBlocking {
                OffloadPermissionManager.requestSettingsGate(
                    OffloadPermissionManager.SettingsGateRequest(
                        ...
                        message = "Fin needs Notification access to read the status-bar notifications. Open Settings and enable \"Fin\" under Notification access.",
                        settingsAction = FinNotificationListenerService.SETTINGS_ACTION,
                        requiresPackageUri = false,
                        positiveLabel = "Open Settings",
                    ),
                    check = { FinNotificationListenerService.isEnabled(context) },
                )
            }
            when (result) {
                OffloadPermissionManager.AndroidPermissionResult.GRANTED -> {} // continue
        ...
        val active: Array<StatusBarNotification> =
            FinNotificationListenerService.getActiveNotifications()
                ?: return NativeOffloadResult(
                    77,
                    OffloadOutput.formatBody(
                        JSONObject()
...
Notes:
  ...
  - list needs Notification access (Settings → Apps → Special app
    access → Notification access → Fin). First list call opens it.
"""
    }
}
