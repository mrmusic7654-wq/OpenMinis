package com.mrmusic.fin.sandbox.offload
...
import com.mrmusic.fin.provider.safeOptString
import com.mrmusic.fin.sandbox.NativeOffloadHandler
import com.mrmusic.fin.sandbox.NativeOffloadRequest
import com.mrmusic.fin.sandbox.NativeOffloadResult
...
class ModelUseOffloadHandler(
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        ...
        if (hasAudioInput && provider !is com.mrmusic.fin.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.mrmusic.fin.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.mrmusic.fin.provider.openai.OpenAIProvider)?.let { openAI ->
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.mrmusic.fin.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.mrmusic.fin.data.model.ImageEndpointMode.auto
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.mrmusic.fin.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        ...
        provider: com.mrmusic.fin.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.mrmusic.fin.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.mrmusic.fin.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.mrmusic.fin.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.mrmusic.fin.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.mrmusic.fin.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.mrmusic.fin.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.mrmusic.fin.data.model.LLMError.ProviderError ||
                e is com.mrmusic.fin.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.mrmusic.fin.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.mrmusic.fin.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.mrmusic.fin.offload.ModelUseManager.entryDict(
            entry,
