...
/**
 * Long-press + drag selection gesture for FinTextKit. Mounted on the
 ...
 * [FinMarkdownTextToolbarHost] (which is driven by Compose's
 ...
 * the older [FinMarkdownTextToolbar] so the per-button UX stays the
...
data class SelectionToolbarActions(
     ...
     * through Fin TTS. Null hides the button. Mirrors iOS's "Read Aloud /
    ...
    Popup(
        ...
        Surface(
            ...
            Row(
                ...
                val labelCopy = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_copy)
                val labelAddToInput = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_add_to_chat_input)
                val labelCopyMarkdown = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_copy_markdown)
                val labelCopyRichText = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_copy_rich_text)
                val labelReadAloud = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_read_aloud)
                val toastCopiedAsMarkdown = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_copied_as_markdown_toast)
                val toastCopiedAsRichText = androidx.compose.ui.res.stringResource(com.mrmusic.fin.R.string.selection_copied_as_rich_text_toast)
                // [T-android-markdown-table-copy-actions] Copy Table / Copy
                // Table Image, when the selected message contains a table.
                //
                // This is the toolbar that ACTUALLY shows for a FinTextKit
                // selection (a table-cell long-press). The table actions were
                // previously wired only into FinMarkdownTextToolbar — the
                // Compose-SelectionContainer toolbar that this FinTextKit
                ...
                val labelCopyTable = androidx.compose.ui.res.stringResource(
                    com.mrmusic.fin.R.string.markdown_table_copy_table)
                val labelCopyTableImage = androidx.compose.ui.res.stringResource(
                    com.mrmusic.fin.R.string.markdown_table_copy_table_image)
                    ...
                    add(SelectionAction(labelCopy) {
                        ...
                        if (text.isNotEmpty()) {
                            ...
                            toast(context.getString(
                                com.mrmusic.fin.R.string.selection_copied_toast, preview(text)))
                        }
                        controller.clearSelection()
                    })
                ...
                inlineItems.forEachIndexed { index, item ->
                    if (index > 0) FinToolbarDivider()
                    FinToolbarButton(label = item.label, onClick = item.onClick)
                }
                if (overflowItems.isNotEmpty()) {
                    FinToolbarDivider()
                    var overflowOpen by remember(items.size) { mutableStateOf(false) }
                    Box {
                        FinToolbarButton(label = "⋯") { overflowOpen = true }
...
@Composable
private fun FinToolbarButton(label: String, onClick: () -> Unit) {
    Text(
        text = label,
