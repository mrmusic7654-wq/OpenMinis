...
/**
 * FinTextKit — a self-contained text selection layer that survives
 ...
 *
 * FinTextKit avoids that by keeping the *authoritative selection state*
...
class SelectionController {
     ...
     * (e.g. a user message bubble, which intentionally skips FinTextKit
    ...
    private fun spliceNonShardSpan(
        ...
        // composables (no FinTextKit shard), so the per-shard walk drops them
     ...
     * markdown slice contain a block that doesn't register as a FinTextKit
 ...
 */
val LocalFinSelectionController = compositionLocalOf<SelectionController?> { null }
...
fun ProvideSelectionController(
    ...
    CompositionLocalProvider(LocalFinSelectionController provides controller) {
        content()
    }
}
...
fun RegisterSelectionShard(shard: TextShard?) {
    val controller = LocalFinSelectionController.current ?: return
