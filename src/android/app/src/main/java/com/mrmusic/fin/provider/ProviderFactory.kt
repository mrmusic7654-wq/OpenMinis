package com.mrmusic.fin.provider
...
import com.mrmusic.fin.provider.anthropic.AnthropicProvider
import com.mrmusic.fin.provider.gemini.GeminiProvider
import com.mrmusic.fin.provider.openai.OpenAIProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.mrmusic.fin.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.mrmusic.fin.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.mrmusic.fin.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.mrmusic.fin.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
                    )
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.mrmusic.fin.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.mrmusic.fin.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.mrmusic.fin.auth.KimiOAuthManager(context, instance.id)
