...
import com.mrmusic.fin.accessibility.AccessibilityRecoveryManager
...
import com.mrmusic.fin.deeplink.DeepLinkHandler
import com.mrmusic.fin.logging.AppLogger
import com.mrmusic.fin.service.SessionActivityTracker
import com.mrmusic.fin.ui.navigation.AppNavigation
import com.mrmusic.fin.ui.navigation.Routes
import com.mrmusic.fin.ui.navigation.safeNavigate
import com.mrmusic.fin.ui.settings.KEY_FONT_APP_BASE
import com.mrmusic.fin.ui.settings.KEY_KEEP_SCREEN_AWAKE
import com.mrmusic.fin.ui.settings.KEY_LANGUAGE
import com.mrmusic.fin.ui.settings.KEY_LAUNCH_SESSION
import com.mrmusic.fin.ui.settings.KEY_THEME_MODE
import com.mrmusic.fin.ui.settings.PREF_APPEARANCE
import com.mrmusic.fin.ui.settings.getAppearancePrefs
import com.mrmusic.fin.ui.settings.fontScaleForLevel
import com.mrmusic.fin.ui.settings.keepScreenAwakeEnabled
import com.mrmusic.fin.ui.theme.AppTheme
import com.mrmusic.fin.ui.theme.FinTheme
...
class MainActivity : ComponentActivity() {
     ...
     * [com.mrmusic.fin.ui.navigation.AppNavigation]'s LaunchedEffect that
    ...
    override fun attachBaseContext(newBase: android.content.Context) {
        super.attachBaseContext(com.mrmusic.fin.i18n.LocaleWrap.wrap(newBase))
    }

    /**
     * T-android-safemode-lateinit-crash: escape hatch for a process whose
     * [FinApp.onCreate] early-returned under safe-mode.
     *
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // AlertDialog (system-level) which doesn't touch FinApp state.
        ...
        // dismisses the share dialog, but FinApp.onCreate already
        ...
        // genuinely unsafe.
        val minisApp = application as? FinApp
        if (minisApp == null || !minisApp.subsystemsInitialized) {
            android.util.Log.w(
                "MainActivity",
                "app subsystems not initialized (safeMode=" +
                    "${com.mrmusic.fin.crash.CrashFrequencyDetector.isSafeMode()}) — " +
                    "showing crash share dialog and finishing",
            )
            com.mrmusic.fin.crash.CrashFrequencyDetector.maybeShowOnActivity(
                activity = this,
                ...
                // tapped, reading as "Fin won't open at all". The process
        ...
        restoredChatSessionId = savedInstanceState?.getString(KEY_CURRENT_CHAT_SESSION_ID)
            ?.takeUnless {
                com.mrmusic.fin.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(this)
            }
        ...
        // (FinApp.swift:279) rather than keying off a launch parameter.
        ...
        com.mrmusic.fin.share.ShareCoordinator.processPendingShare(this)
        ...
        if (BuildConfig.DEBUG) {
            com.mrmusic.fin.debug.DebugRPCHandler.currentActivity = java.lang.ref.WeakReference(this)
        }

        // Non-null and fully initialized — proven by the guard above.
        val app = requireNotNull(application as? FinApp)
        ...
        setContent {
            ...
            // [T-app-themes] Visual theme (Space / Sun / Moon / …) — orthogonal
            // to themeMode, which only decides light vs dark.
            var appTheme by remember {
                mutableStateOf(AppTheme.fromId(prefs.getString(AppTheme.KEY_APP_THEME, null)))
            }
            ...
            DisposableEffect(prefs) {
                val listener = SharedPreferences.OnSharedPreferenceChangeListener { sp, key ->
                    when (key) {
                        ...
                        AppTheme.KEY_APP_THEME ->
                            appTheme = AppTheme.fromId(sp.getString(AppTheme.KEY_APP_THEME, null))
            ...
            FinTheme(darkTheme = darkTheme, appTheme = appTheme, fontScale = fontScale) {
                ...
                // agent triggers a change. Mirrors iOS FinApp.swift
                // root-level `.sheet(item: gate.pending)`.
                com.mrmusic.fin.ui.settings.ConfigConfirmDialogHost()
            }
        }
    }
     ...
     * is already covered by [com.mrmusic.fin.ui.navigation.AppNavigation]
