...
import com.mrmusic.fin.data.db.AppDatabase
import com.mrmusic.fin.data.repository.BackgroundSettingsRepository
import com.mrmusic.fin.data.repository.ChatRepository
import com.mrmusic.fin.data.repository.EnvVarRepository
import com.mrmusic.fin.data.MountedFoldersStore
import com.mrmusic.fin.data.repository.MemoryRepository
import com.mrmusic.fin.data.repository.ProviderRepository
import com.mrmusic.fin.data.repository.WebAppShortcutRepository
import com.mrmusic.fin.data.repository.MCPRepository
import com.mrmusic.fin.data.repository.SkillRepository
import com.mrmusic.fin.notification.BackgroundTaskNotifier
import com.mrmusic.fin.logging.AppLogger
import com.mrmusic.fin.network.NetworkMonitor
import com.mrmusic.fin.offload.OffloadPermissionManager
import com.mrmusic.fin.provider.ModelsDevApi
import com.mrmusic.fin.sandbox.ExecutionCoordinator
import com.mrmusic.fin.sandbox.MountedFolderCoordinator
import com.mrmusic.fin.sandbox.NativeOffloadServer
import com.mrmusic.fin.sandbox.PRootKernel
import com.mrmusic.fin.sandbox.RootfsManager
import com.mrmusic.fin.sandbox.offload.AccessibilityOffloadHandler
import com.mrmusic.fin.sandbox.offload.AlarmOffloadHandler
import com.mrmusic.fin.sandbox.offload.BrowserUseOffloadHandler
import com.mrmusic.fin.sandbox.offload.CalendarOffloadHandler
import com.mrmusic.fin.sandbox.offload.ClipboardOffloadHandler
import com.mrmusic.fin.sandbox.offload.ContactsOffloadHandler
import com.mrmusic.fin.sandbox.offload.DeviceOffloadHandler
import com.mrmusic.fin.sandbox.offload.LocationOffloadHandler
import com.mrmusic.fin.sandbox.offload.ModelUseOffloadHandler
import com.mrmusic.fin.sandbox.offload.SessionsOffloadHandler
import com.mrmusic.fin.sandbox.offload.ShizukuOffloadHandler
import com.mrmusic.fin.sandbox.offload.NotificationOffloadHandler
import com.mrmusic.fin.sandbox.offload.OpenOffloadHandler
import com.mrmusic.fin.sandbox.offload.PhotosOffloadHandler
import com.mrmusic.fin.sandbox.offload.PlayerOffloadHandler
import com.mrmusic.fin.sandbox.offload.SpeakOffloadHandler
import com.mrmusic.fin.sandbox.offload.SpeechOffloadHandler
import com.mrmusic.fin.sandbox.offload.WeatherOffloadHandler
import com.mrmusic.fin.service.SessionActivityTracker
import com.mrmusic.fin.ui.MinisImageFetcher
import kotlinx.coroutines.launch

class FinApp : Application(), ImageLoaderFactory {
    /**
    ...
    override fun onCreate() {
        ...
        if (ACRA.isACRASenderServiceProcess()) {
            Log.i("FinApp", "skipping app init in :acra reporter process")
            return
        }
        ...
        com.mrmusic.fin.data.FastModePrefs.prime(this)
        ...
        // Activity context.
        com.mrmusic.fin.data.AutoCompactPrefs.prime(this)
        ...
        // libs first execute.
        try {
            com.mrmusic.fin.crash.NativeCrashHandler.install(
                java.io.File(filesDir, "logs"),
            )
        } catch (t: Throwable) {
            Log.w("FinApp", "NativeCrashHandler install failed: ${t.message}")
        }
        ...
        try {
            ...
            Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
                try {
                    ...
                    if (isFgsTimeout) {
                        ...
                        runCatching {
                            val intent = Intent(this, com.mrmusic.fin.service.AgentForegroundService::class.java)
                            stopService(intent)
                        }
                    }
                } catch (t: Throwable) {
                    Log.w("FinApp", "FGS-timeout handler internal failure: ${t.message}")
                }
            ...
            Log.w("FinApp", "install FGS-timeout handler failed: ${t.message}")
        }
        ...
        com.mrmusic.fin.crash.CrashFrequencyDetector.checkAtLaunch(this)
        ...
        if (com.mrmusic.fin.crash.CrashFrequencyDetector.isSafeMode()) {
            Log.w("FinApp", "safe-mode ON — skipping app subsystem init")
            return
        }
        ...
        // `LoggingManager.startIfEnabled()` (called from FinApp.swift:143).
        AppLogger.init(this)
        ...
        // [com.mrmusic.fin.diagnostics.LaunchCycleBeacon].
        try {
            com.mrmusic.fin.diagnostics.LaunchCycleBeacon.recordLaunch(this)
        } catch (t: Throwable) {
            Log.w("FinApp", "LaunchCycleBeacon.recordLaunch failed: ${t.message}")
        }
        ...
        com.mrmusic.fin.diagnostics.HangDetector.start(this)
        ...
        subsystemsInitialized = true
            ...
            Log.e("FinApp", "subsystem init failed — app will start in degraded mode", t)
            return
        }
        ...
        com.mrmusic.fin.agent.SoulStore.ensureExists(this)
        com.mrmusic.fin.agent.SoulStore.refreshCache(this)
        ...
        com.mrmusic.fin.config.MinisConfigPermissionStore.init(this)
        com.mrmusic.fin.config.audit.ConfigAuditLog.init(this)
        com.mrmusic.fin.config.ConfigRegistry.init(
        ...
        com.mrmusic.fin.data.EnvVarPrivacyStore.init(this)
        com.mrmusic.fin.data.EnvVarRedactor.envVarRepository = envVarRepository
        ...
        // T323: UI-layer automation backed by FinAccessibilityService.
        ...
        NativeOffloadServer.register(
            "minis-config",
            com.mrmusic.fin.sandbox.offload.ConfigOffloadHandler(),
        )
        ...
        NativeOffloadServer.register(
            "minis-scheduled",
            com.mrmusic.fin.sandbox.offload.ScheduledTaskOffloadHandler(this),
        )
        ...
        com.mrmusic.fin.offload.ShizukuManager.init(this)
        ...
        if (BuildConfig.DEBUG) {
            NativeOffloadServer.register(
                "minis-debug",
                com.mrmusic.fin.sandbox.offload.DebugOffloadHandler(this),
            )
        }
        ...
        com.mrmusic.fin.service.SessionBadgeStore.init(this)
        ...
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
            ...
            com.mrmusic.fin.service.SessionBadgeStore.reconcileInterruptedSessions(interrupted - active)
        }
        ...
        val configConfirmNotifier = com.mrmusic.fin.notification.ConfigConfirmNotifier(
            context = this,
            backgroundSettings = backgroundSettingsRepository,
            isAppForeground = ::isAppForeground,
        )
        com.mrmusic.fin.config.confirm.ConfigConfirmationGate.backgroundNotifier = {
            configConfirmNotifier.notifyIfBackgrounded(it)
        }
        com.mrmusic.fin.config.confirm.ConfigConfirmationGate.cancelNotification = {
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            ...
            override fun onActivityResumed(activity: Activity) {
                ...
                com.mrmusic.fin.crash.CrashFrequencyDetector.maybeShowOnActivity(activity)
            ...
            override fun onActivityStopped(activity: Activity) {
                ...
                if (foregroundActivityCount == 0) {
                    ...
                    com.mrmusic.fin.config.confirm.ConfigConfirmationGate.notifyPending()
                }
            }
        ...
        com.mrmusic.fin.speech.SpeechRecognitionManager.init(this)

        // Refresh model lists once per calendar day (mirrors iOS FinApp.swift).
                ...
                when (intent.action) {
                    Intent.ACTION_TIMEZONE_CHANGED -> scope.launch {
                        try {
                            ...
                            Log.w("FinApp", "broadcastTimezoneChange failed: ${t.message}")
                        }
                    }
                    android.net.Proxy.PROXY_CHANGE_ACTION -> scope.launch {
                        try {
                            ...
                            Log.w("FinApp", "broadcastProxyChange failed: ${t.message}")
                        }
                    }
                }
            }
        ...
        if (BuildConfig.DEBUG) {
            try {
                com.mrmusic.fin.debug.DebugServer(this).start()
            } catch (e: Exception) {
                Log.w("FinApp", "Failed to start debug server: ${e.message}")
            }
        }

        // T268: one-shot migration of pre-T266 internal alarms into the
        // system Clock app. Pre-T266 builds wrote alarms into Fin's own
        // SharedPreferences + AlarmManager; T266 retired that path but old
        // installs still have ghost entries that fire only inside Fin.
        ...
        runCatching { migrateGhostAlarms() }
            .onFailure { Log.w("FinApp", "ghost alarm migration failed: ${it.message}") }
    }
    ...
    private fun migrateGhostAlarms() {
        ...
        prefs.edit().remove("alarms_json").apply()
        Log.i("FinApp", "T268 ghost alarm migration: migrated=$migrated skipped=$skipped (prefs cleared)")
    }
    ...
    override fun onTrimMemory(level: Int) {
        ...
        Log.i("FinApp", "onTrimMemory(level=$level): releasing formula bitmap caches")
        runCatching { com.mrmusic.fin.ui.chat.KatexWebViewPool.evictAll() }
            .onFailure { Log.w("FinApp", "KatexWebViewPool.evictAll failed: ${it.message}") }
        runCatching { com.mrmusic.fin.ui.markdown.KaTeXRendererCache.evictAll() }
            .onFailure { Log.w("FinApp", "KaTeXRendererCache.evictAll failed: ${it.message}") }

        if (level >= TRIM_MEMORY_COMPLETE) {
            Log.i("FinApp", "onTrimMemory(level=$level): tearing down the offscreen KaTeX WebView")
            runCatching { com.mrmusic.fin.ui.chat.KatexWebViewPool.releaseWebView() }
                .onFailure { Log.w("FinApp", "KatexWebViewPool.releaseWebView failed: ${it.message}") }
        }
    }
    ...
    override fun onTerminate() {
        ...
        try {
            com.mrmusic.fin.diagnostics.LaunchCycleBeacon.recordCleanExit(this)
        } catch (t: Throwable) {
            Log.w("FinApp", "LaunchCycleBeacon.recordCleanExit failed: ${t.message}")
        }
        super.onTerminate()
    }
