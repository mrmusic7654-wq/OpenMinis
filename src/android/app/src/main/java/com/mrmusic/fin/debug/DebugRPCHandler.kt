...
class DebugRPCHandler(private val context: Context) {
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.mrmusic.fin.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.mrmusic.fin.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.mrmusic.fin.data.UpdateChecker.check()) {
            is com.mrmusic.fin.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.mrmusic.fin.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.mrmusic.fin.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.mrmusic.fin.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.mrmusic.fin.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.mrmusic.fin.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.mrmusic.fin.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.mrmusic.fin.data.UpdateChecker.download(context, url)) {
            is com.mrmusic.fin.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.mrmusic.fin.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.mrmusic.fin.data.UpdateChecker.canInstall(context)
        return try {
            com.mrmusic.fin.data.UpdateChecker.installApk(context, file)
     ...
     * iCloud-equivalent built into FinApp, so we report disabled and an
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.mrmusic.fin.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.mrmusic.fin.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.mrmusic.fin.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.mrmusic.fin.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.mrmusic.fin.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.mrmusic.fin.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.mrmusic.fin.sandbox.offload.ModelUseOffloadHandler]
    ...
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.mrmusic.fin.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.mrmusic.fin.FinApp
        val handler = com.mrmusic.fin.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.mrmusic.fin.sandbox.NativeOffloadRequest(
            pid = -1,
     ...
     * [com.mrmusic.fin.sandbox.offload.SessionsOffloadHandler] for e2e
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.mrmusic.fin.FinApp
        val handler = com.mrmusic.fin.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.mrmusic.fin.sandbox.NativeOffloadRequest(
            pid = -1,
        ...
        return when (sub) {
            "set" -> {
                ...
                kotlinx.coroutines.runBlocking {
                    com.mrmusic.fin.config.ConfigBridge.performWriteBatch(
                        items = items,
            ...
            "get" -> {
                ...
                com.mrmusic.fin.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", com.mrmusic.fin.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
                ...
                val known = com.mrmusic.fin.config.ConfigBridge.allTopics()
                    .let { arr -> (0 until arr.length()).any { arr.optString(it) == topic } }
                if (!known) {
                    throw RPCException(-32602, "Unknown topic '$topic' — call subcommand=topics")
                }
                val fields = com.mrmusic.fin.config.ConfigBridge.fieldsForTopic(topic)
                JSONObject().apply {
                    put("ok", true)
                    put("topic", topic)
            ...
            "audit-list" -> {
                ...
                com.mrmusic.fin.config.ConfigBridge.auditList(limit, scope)
            }
