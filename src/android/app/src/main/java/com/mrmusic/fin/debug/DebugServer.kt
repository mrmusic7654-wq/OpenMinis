...
class DebugServer(
    ...
    fun start() {
        ...
        acceptJob = scope.launch {
            try {
                ...
                Log.i(TAG, "Remote (non-loopback) clients must send X-Fin-Token: $authToken")
        ...
        socket.use { s ->
            try {
                ...
                while (true) {
                    ...
                    // [T-android-debugserver-auth] Token via X-Fin-Token or
                ...
                if (!isAuthorized(isLoopback, providedToken, authToken)) {
                    ...
                    sendResponse(writer, 401, rpcHandler.errorJSON(-32000, "Unauthorized — send X-Fin-Token (see `adb shell run-as com.mrmusic.fin cat files/debug_server_token`)"))
                    return
                }
    ...
    private fun schemaJSON(): String {
        ...
        return """{"app":"FinApp","platform":"android","version":"$version",""" +
    ...
    private fun sendResponse(
        ...
        writer.print("Access-Control-Allow-Headers: Content-Type, X-Fin-Token, Authorization\r\n")
        writer.print("\r\n")
        writer.print(body)
        writer.flush()
    }
