...
import com.mrmusic.fin.accessibility.FinAccessibilityService
import com.mrmusic.fin.accessibility.NodeRegistry
 ...
 * [FinAccessibilityService]. Mirrors the design doc
...
First-run: enable "Fin" under Settings → Accessibility, then `service ping`.
"""
...
"""
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
            ...
            if (!usable) {
                return err(
                    ...
                    "Accessibility permission was revoked (this happens after a force-stop). " +
                        "Re-enable Fin under Settings → Accessibility, or use Settings → " +
        ...
        com.mrmusic.fin.service.SessionActivityTracker.updateToolStatus(
        ...
        return try {
            ...
            err(args, "SERVICE_NOT_RUNNING",
                e.message ?: "Accessibility service is not running. Enable Fin under Settings → Accessibility.",
                exit = 77)
        } catch (e: Throwable) {
    ...
    private fun serviceSub(args: OffloadArgs): NativeOffloadResult {
        return when (args.positional.getOrNull(1)) {
            ...
            "status" -> {
                val svc = FinAccessibilityService.getInstance()
                val data = JSONObject()
                    .put("running", svc != null)
                    .put("serviceName", FinAccessibilityService.SERVICE_ID)
            ...
            "ping" -> {
                if (FinAccessibilityService.getInstance() != null)
                    NativeOffloadResult(0, "✓ Accessibility service is running\n")
                else
                    NativeOffloadResult(77, "✗ Accessibility service is not running — go to Settings → Accessibility → Fin to enable\n")
            }
    ...
    private fun tapXYRaw(svc: FinAccessibilityService, x: Int, y: Int, args: OffloadArgs): NativeOffloadResult {
    ...
    private fun resolveTargetEditable(svc: FinAccessibilityService, args: OffloadArgs): AccessibilityNodeInfo? {
    ...
    private fun treeSignature(svc: FinAccessibilityService): Int {
        var h = 0
        for (root in svc.rootNodes()) {
    ...
    private fun eventWatch(args: OffloadArgs, once: Boolean): NativeOffloadResult {
        ...
        val collected = java.util.concurrent.ConcurrentLinkedQueue<FinAccessibilityService.RecordedEvent>()
        val listener: (FinAccessibilityService.RecordedEvent) -> Unit = { collected.offer(it) }
    ...
    private fun svcOrThrow(): FinAccessibilityService =
        FinAccessibilityService.getInstance()
            ?: throw NotRunning("Accessibility service is not running. Enable Fin under Settings → Accessibility.")
