...
class ShareReceiverActivity : ComponentActivity() {
    ...
    private fun promptProviderImportOrAttach(
        ...
        return try {
            android.app.AlertDialog.Builder(this)
                .setTitle(getString(com.mrmusic.fin.R.string.share_provider_json_title))
                .setMessage(getString(com.mrmusic.fin.R.string.share_provider_json_message))
                .setPositiveButton(getString(com.mrmusic.fin.R.string.share_provider_json_import)) { _, _ ->
                ...
                .setNegativeButton(getString(com.mrmusic.fin.R.string.share_provider_json_attach)) { _, _ ->
                    finishWithAttachmentFlow(items)
                }
                .setOnCancelListener {
    ...
    private fun importProviderJson(json: String) {
        ...
        val repo = (applicationContext as? com.mrmusic.fin.FinApp)?.providerRepositoryOrNull
        if (repo == null) {
            AppLogger.warning(TAG, "providerRepository unavailable; cannot import")
            toast(getString(com.mrmusic.fin.R.string.share_provider_json_import_failed))
            return
        }
        val label = repo.importInstanceJSON(json)
        if (label != null) {
            AppLogger.info(TAG, "imported provider \"$label\" from shared JSON")
            toast(getString(com.mrmusic.fin.R.string.share_provider_json_imported, label))
        } else {
     ...
     * failure to start it.
     *
     * Field report (vivo V2352A / Android 14): sharing into Fin crashed the
    ...
    private fun launchMainActivity() {
        ...
        when (outcome) {
            ...
            ShareHandoffPolicy.Outcome.FAILED -> {
                ...
                try {
                    toast(getString(com.mrmusic.fin.R.string.share_open_app_failed))
                } catch (t: Throwable) {
    ...
    private fun mainActivityIntent(flags: Int): Intent =
        Intent(this, Class.forName("com.mrmusic.fin.MainActivity")).apply {
            addFlags(flags)
