...
import com.mrmusic.fin.data.model.normalizeModalityName
...
internal suspend fun performTest(
    ...
    if (kind == QuickTestKind.SPEECH_OUT || kind == QuickTestKind.TRANSCRIPTION) {
        val voice = com.mrmusic.fin.provider.voice.VoiceProviderFactory.make(instance, apiKey)
        ...
        return@withContext when (kind) {
            QuickTestKind.SPEECH_OUT -> {
                if (!voice.supportsVoiceOutput) {
                    ...
                    runCatching {
                        ...
                        val data = voice.synthesize(
                            com.mrmusic.fin.provider.voice.VoiceOutputRequest(
                ...
                if (!voice.supportsVoiceInput) {
                    ...
                    val spoken = "Hello from Fin, testing speech to text."
                    ...
                    runCatching {
                        val resp = voice.transcribe(
                            com.mrmusic.fin.provider.voice.VoiceInputRequest(
                                audioData = clip,
