package com.mrmusic.fin.ui.navigation
...
import com.mrmusic.fin.deeplink.DeepLinkAction
import com.mrmusic.fin.deeplink.DeepLinkCoordinator
import com.mrmusic.fin.ui.settings.KEY_LAUNCH_SESSION
import com.mrmusic.fin.ui.settings.getAppearancePrefs
...
import com.mrmusic.fin.ui.chat.ChatScreen
import com.mrmusic.fin.ui.sessions.SessionListScreen
import com.mrmusic.fin.ui.settings.AboutScreen
import com.mrmusic.fin.ui.settings.AddAgentLoopGroupsScreen
import com.mrmusic.fin.ui.settings.AddAgentLoopModelsScreen
import com.mrmusic.fin.ui.settings.AddCustomModelScreen
import com.mrmusic.fin.ui.settings.BackgroundSettingsScreen
import com.mrmusic.fin.ui.settings.AddModelsToGroupScreen
import com.mrmusic.fin.ui.settings.ShadowVoiceDetailScreen
import com.mrmusic.fin.ui.settings.AddProviderScreen
import com.mrmusic.fin.ui.settings.ModelEntryDetailScreen
import com.mrmusic.fin.ui.settings.ModelGroupDetailScreen
import com.mrmusic.fin.ui.settings.ModelGroupsScreen
import com.mrmusic.fin.ui.settings.ProviderDetailScreen
import com.mrmusic.fin.ui.settings.ProviderListScreen
import com.mrmusic.fin.ui.sandbox.FileBrowserScreen
import com.mrmusic.fin.ui.sandbox.FileBrowserViewModel
import com.mrmusic.fin.ui.sandbox.FileItem
import com.mrmusic.fin.ui.sandbox.FilePreviewScreen
import com.mrmusic.fin.ui.sandbox.RootfsManagementScreen
import com.mrmusic.fin.ui.settings.EnvironmentVariablesScreen
import com.mrmusic.fin.ui.settings.AppearanceScreen
import com.mrmusic.fin.ui.settings.SettingsScreen
import com.mrmusic.fin.ui.settings.SystemPermissionsScreen
import com.mrmusic.fin.ui.settings.SessionStorageDetailScreen
import com.mrmusic.fin.ui.settings.SkillDetailScreen
import com.mrmusic.fin.ui.settings.StorageManagementScreen
import com.mrmusic.fin.ui.settings.SkillFileViewerScreen
import com.mrmusic.fin.ui.settings.UsageStatsScreen
import com.mrmusic.fin.ui.settings.MinisSkillsBrowserScreen
import com.mrmusic.fin.ui.settings.MountDetailScreen
import com.mrmusic.fin.ui.settings.MountedFoldersScreen
import com.mrmusic.fin.ui.settings.SharedFolderDetailScreen
import com.mrmusic.fin.ui.settings.SharedFoldersScreen
import com.mrmusic.fin.ui.settings.SkillsManagementScreen
...
import com.mrmusic.fin.ui.settings.LogDetailScreen
import com.mrmusic.fin.ui.settings.LogManagementScreen
import com.mrmusic.fin.ui.settings.MemoryFileEditScreen
import com.mrmusic.fin.ui.settings.MemoryManagementScreen
import com.mrmusic.fin.ui.settings.OffloadPermissionScreen
import com.mrmusic.fin.ui.settings.ShizukuPermissionScreen
import com.mrmusic.fin.sandbox.RootfsManager
import com.mrmusic.fin.sandbox.TerminalSession
import com.mrmusic.fin.ui.terminal.TerminalScreen
import com.mrmusic.fin.ui.onboarding.OnboardingModelSelectionScreen
...
fun AppNavigation(
    ...
    // T219-5: use the application-scoped singleton from FinApp so UI
    ...
    // the application-scoped singleton in FinApp.
    val mountedFoldersStore = remember {
        (context.applicationContext as com.mrmusic.fin.FinApp).mountedFoldersStore
    }
    ...
    LaunchedEffect(Unit) {
        ...
        val hasPendingShare =
            com.mrmusic.fin.share.ShareCoordinator.bufferVersion.value > 0
        ...
        val mode = if (
            com.mrmusic.fin.diagnostics.HangDetector.shouldForceHomeOnLaunch(context) ||
            com.mrmusic.fin.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(context) ||
            com.mrmusic.fin.diagnostics.LaunchCycleBeacon.shouldForceHomeOnLaunch()
        ) 3 else rawMode
    ...
    NavHost(
            ...
            SharedFolderDetailScreen(
                ...
                onBrowseFiles = {
                    ...
                    val label = when (folderId) {
                        "shared" -> ctx.getString(com.mrmusic.fin.R.string.shared_folder_name_shared)
                        "skills" -> ctx.getString(com.mrmusic.fin.R.string.shared_folder_name_skills)
                        "memory" -> ctx.getString(com.mrmusic.fin.R.string.shared_folder_name_memory)
                        else -> folderId
                    }
        ...
        composable(
            ...
            val category = com.mrmusic.fin.ui.sandbox.MirrorCatalog.categoryFromKey(categoryKey)
                ?: return@composable
            com.mrmusic.fin.ui.sandbox.MirrorCategoryDetailScreen(
                category = category,
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(Routes.MCP) {
            if (mcpRepository != null) {
                com.mrmusic.fin.ui.settings.MCPIntegrationsScreen(
                    mcpRepository = mcpRepository,
                    onBack = { navController.safePopBackStack() },
                    envVarRepository = envVarRepository,
                )
        ...
        composable(Routes.SOUL) {
            com.mrmusic.fin.ui.settings.SoulSettingsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(Routes.SCHEDULED_TASKS) {
            com.mrmusic.fin.ui.scheduled.ScheduledTasksScreen(
                onBack = { navController.safePopBackStack() },
                onEditTask = { taskId ->
        ...
        composable(
            ...
            com.mrmusic.fin.ui.scheduled.ScheduledTaskRunsScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
        ...
        composable(
            ...
            com.mrmusic.fin.ui.scheduled.ScheduledTaskEditScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
