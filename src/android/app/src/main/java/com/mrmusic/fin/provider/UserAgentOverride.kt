 ...
 * null/blank, we now apply [FinUserAgent.DEFAULT] instead of leaving
 * the builder UA-less (which lets OkHttp insert its own `okhttp/4.12.0`).
 * The default carries the Fin version so request logs upstream can be
...
fun Request.Builder.applyUserAgentOverride(
    ...
    defaultUserAgent: String? = FinUserAgent.DEFAULT,
): Request.Builder {
...
/**
 * [T-android-default-ua] Branded User-Agent used by every Fin-originated
 * outbound request that doesn't have a SDK-specific UA requirement.
 *
 * Format mirrors iOS exactly:
 *   `Fin/<version> (Android <release>; <model>)`
 *
 * e.g. `Fin/0.14-preview (Android 13; Pixel 4a)` — same shape as iOS's
 * `Fin/1.10 (iOS 26.5; iPhone)`.
 *
...
object FinUserAgent {
    val DEFAULT: String by lazy {
        ...
        "Fin/${BuildConfig.VERSION_NAME} (Android $release; $model)"
    }
}
