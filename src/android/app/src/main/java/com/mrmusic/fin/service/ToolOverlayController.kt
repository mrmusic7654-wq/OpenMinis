 ...
 * shows the Fin launcher logo, the running tool's label, and a short
 * status — same data the FGS notification reads
 * (`SessionActivityTracker.currentToolName` / `currentToolStatus`).
 *
 * T-bg-overlay-polish: the per-tool icon was replaced with the Fin
 ...
 *
 *   - Fin foreground OR no tool running OR user toggle OFF OR no perm → hidden
 *   - otherwise → shown, content updated on each tool-status change
 *
 * Position persists in [com.mrmusic.fin.data.repository.BackgroundSettingsRepository]
 * across process restarts; first run defaults to the bottom-left corner
 * (10 dp from each edge). Tap dismisses to bring Fin back to the
...
class ToolOverlayController(private val context: Context) {
    ...
    private val backgroundRepo =
        (context.applicationContext as FinApp).backgroundSettingsRepository
    ...
    private fun updateContent(
        ...
        // through verbatim would render a noisy "Fin / Idle" capsule that
    ...
    private fun onTap() {
        try {
            ...
            val launchIntent = Intent(
                context,
                Class.forName("com.mrmusic.fin.MainActivity"),
            ).apply {
     ...
     * "Fin is using " prefix — the overlay capsule is tight, so we just
