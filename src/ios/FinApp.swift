...
struct FinApp: App {
    ...
    /// [T-app-themes] Visual theme (Space / Sun / Moon / …). Observed here so a
    /// change re-keys the root exactly like a language change does — see the
    /// `.id(...)` below.
    ...
    init() {
        ...
        // first render instead of the `.default` stub ("Fin"). Without this the
        ...
        AgentLiveActivityManager.shared.cleanupStaleActivities(source: "FinApp.init")
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                ...
                //
                // [T-app-themes] The visual theme rides on the same key: the
                // chat/settings surfaces read their colours through
                // `ChatColors` / `ThemePalette` inside view bodies, and a
                // remount is the only way to make every one of them
                // re-resolve at once. `AppearanceSettingsView` arms the same
                // `pendingSettingsReopen` flag before a theme change, so the
                // user lands back on the Appearance page afterwards.
                .id("\(appLanguage)|\(themeManager.theme.rawValue)")
                // Global tint / accent / window tint for the active theme.
                .appThemeRoot()
                ...
                .onAppear {
                    ...
                    if #available(iOS 17.0, *) {
                        FinShortcutsProvider.updateAppShortcutParameters()
                    }
    ...
    private static let fileProviderDomain = NSFileProviderDomain(
        identifier: NSFileProviderDomainIdentifier("com.mrmusic.fin.files"),
        displayName: "Fin"
    )
    ...
    private static func migrateSharedDirToAppGroup() {
        ...
        let container = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.mrmusic.fin")!
    ...
    private static func logFPSyncTracePaths() {
        let fm = FileManager.default
        let groupID = "group.com.mrmusic.fin"
