...
enum ProviderMigration {
    ...
    private static let migrationKey = "com.mrmusic.fin.provider-migration-v1-done"
    private static let oauthMigrationKey = "com.mrmusic.fin.provider-migration-oauth-v2-done"
    ...
    private static func migrate(store: ProviderConfigStore) {
        ...
        // API Key
        if let key = readLegacyKeychain(service: "com.mrmusic.fin.anthropic-api-key") {
        ...
        // API Key
        if let key = readLegacyKeychain(service: "com.mrmusic.fin.gemini-api-key") {
            let instance = ProviderInstance(
        ...
        // API Key
        if let key = readLegacyKeychain(service: "com.mrmusic.fin.openai-api-key") {
            let instance = ProviderInstance(
    ...
    private static func loadLegacyAgentModelSettings() -> LegacyAgentModelSettings {
        let key = "com.mrmusic.fin.agent-model-settings"
    ...
    private static func isLegacyActiveProvider(_ rawValue: String) -> Bool {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: "com.mrmusic.fin.active-provider",
