...
final class SystemVoiceProvider: NSObject, VoiceInputCapable, VoiceOutputCapable {
    ...
    static func sampleText(forLanguage langCode: String?) -> String {
        ...
        if lc.hasPrefix("zh") { return "你好，这是 Fin 的语音测试。" }
        if lc.hasPrefix("ja") { return "こんにちは、これは Fin の音声テストです。" }
        if lc.hasPrefix("ko") { return "안녕하세요, Fin 음성 테스트입니다." }
        return "Hi! This is Fin testing this voice."
    }
