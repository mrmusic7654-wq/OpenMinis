...
///
/// Without an explicit header, URLSession sends `Fin/<CFBundleVersion>
/// CFNetwork/… Darwin/…` — i.e. the **build number** (e.g. `Fin/1`), not the
...
enum FinUserAgent {
    ...
    static var `default`: String {
        ...
        let ua = "Fin/\(version) (iOS \(osVersion); \(model))"
        cached = ua
        return ua
    }
}
