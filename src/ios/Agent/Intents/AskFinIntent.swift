...
private let logger = AppLogger(category: "AskFinIntent")

/// Siri-facing "ask Fin" entry point. Unlike `SendPromptIntent` (which runs
...
/// conversation — matching the "Hey Siri, ask Fin to …" experience.
///
...
struct AskFinIntent: AppIntent {
    static var title: LocalizedStringResource = "Ask Fin"
    static var description = IntentDescription("Opens Fin, sends your prompt, and shows the conversation. Starts a new session, or continues an existing one when you pick a session.")
    ...
    @Parameter(title: "Prompt", requestValueDialog: "What would you like to ask Fin?")
    var prompt: String
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        if let session = session {
            ...
            if let placeholder = placeholderSid, let realSid = vm.sessionId {
                BackgroundKeepAliveManager.shared.armEagerlyForShortcut(
                    sessionId: realSid, caller: "AskFinIntent.swap")
                SessionActivityTracker.shared.setInactive(placeholder,
                    source: "AskFinIntent.eager.placeholderSwap")
            }
        }
        ...
        logger.info("AskFin send sid=\(sid.prefix(8)) new=\(session == nil)")
        ...
        return .result(dialog: "On it — opening Fin.")
    }
}
